...
const RequestView = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const fetchedRequests = await RequestController.fetchRequests(currentPage);
                setRequests(fetchedRequests.content.map(req => new Request(req.id, req.message, req.user)));
